...
namespace mat
{
    ...
    {
        EXEC_IF_20(static_assert(CONSTEXPR_MATRIX(M) || RUNTIME_MATRIX(M), "FlattenCopy: M must be CONSTEXPR_MATRIX or RUNTIME_MATRIX"));
    ...
    {
        EXEC_IF_20(static_assert(RUNTIME_MATRIX(M), "ResizeCopy: M must be RUNTIME_MATRIX"));
    ...
    {
        EXEC_IF_20(static_assert(CONSTEXPR_MATRIX(M), "ResizeCopy: M must be CONSTEXPR_MATRIX"));
        if (mat.Width() == W && mat.Height() == H)
        {
            return mat;
        }
