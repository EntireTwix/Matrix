...
namespace mat
{
    ...
    {
        EXEC_IF_NOT_20(static_assert(CONSTEXPR_MATRIX(M) || RUNTIME_MATRIX(M), "Operation: M must be CONSTEXPR_MATRIX or RUNTIME_MATRIX"));
        M res(a);
    ...
    {
        EXEC_IF_NOT_20(static_assert(CONSTEXPR_MATRIX(M) || RUNTIME_MATRIX(M), "OperationMut: M must be CONSTEXPR_MATRIX or RUNTIME_MATRIX"));
        for (size_t i = 0; i < b.Area(); ++i)
        {
