...
namespace mat
{
    ...
    #endif
    constexpr M operator+(const M& mat, T&& v) noexcept { return ScalarOperation(mat, std::move(v), [](copy_fast_cv_t<typename M::type> a, copy_fast_cv_t<T> b){ return a+b; }); }
    template <MATRIX_TYPENAME M, typename T = typename M::type>
    #ifdef __cpp_concepts 
    requires AddableAs<typename M::type, T>
    #endif
    constexpr void operator+=(M& mat, T&& v)  noexcept { ScalarOperationMut(mat, std::move(v), [](typename M::type& a, copy_fast_cv_t<typename M::type> b){ a+=b; }); }
